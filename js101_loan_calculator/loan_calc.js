...
function amortLoanPayment(loanAmount, annualPercentageRate, loanDurationInYears) {
  ...
  if (Number(annualPercentageRate)) {
    monthlyPayment = (Number(loanAmount) * (monthlyInterestRate / 
...
while (inputLoanDurationInYears < 0.5 || invalidNumber(inputLoanDurationInYears)) {
...
prompt(MESSAGES.outputMsg + amortLoanPayment(inputLoanAmount, inputAnnualPercentageRate, inputLoanDurationInYears));

console.log(amortLoanPayment(inputLoanAmount, inputAnnualPercentageRate, inputLoanDurationInYears));
